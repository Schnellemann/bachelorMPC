...
func IncrementMultAndDelay(plotter graph.Interface) {
	for _, delay := range []int{40, 80, 160} {
		incrementMultWithDelay(plotter, time.Duration(delay)*time.Millisecond, 100, 500, 100, makeRandomMultExpression, 2*time.Minute)
	}
	plotter.Plot()
}

func incrementMultWithDelay(plotter graph.Interface, delay time.Duration, start int, end int, increment int, multStrat randomMultMaker, maxExperimentTime time.Duration) {
	fieldRange := 1049
